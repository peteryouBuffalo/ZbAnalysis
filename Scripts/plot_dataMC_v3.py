...
import ConfigParser
#import myutils.util_funcs as utl_func #Hsin-Wei's script
import myutils as utl
import util_funcs as utl_func #Hsin-Wei's script
...
dy_type = 'amcnlo' #mg amcnlo sherpa
...
if dy_type == 'sherpa':
  ss = ['Electron','Muon','DY_Sherpa','TT','ST','WW','WZ','ZZ','ZH']
  ...
  for y in years:
    ...
    for iN in range(len(fNames[s][y])):
      if s not in ['Electron','Muon']:
        ...
        if s == 'DY_sherpa':
         lumiScales[s][y][iN] = scaleToLumi2(fNames[s][y][iN],xSecs[s][y][iN],lumi)
	else:
	 lumiScales[s][y][iN] = utl_func.scaleToLumi1(fNames[s][y][iN],xSecs[s][y][iN],lumi)
...
for r in regions:
  ...
  #for chan in ['Zmm']:
    ...
    for plN in plotNames:
      ...
      if r == 'Z_bjet':
	hN = chan + '_' + r.split('_')[1] + '_afterMET_' + plN
      print hN, plN
      ...
      if dy_type == 'sherpa':
	hDY = getHist(hN,['DY_Sherpa'],fHist,lumiScales)
      ...
      if r.split('_')[1] == '2bjet':
        ...
        if dy_type == 'sherpa':
          hDY1 = getHist(hN1,['DY_Sherpa'],fHist,lumiScales)
          hDY2 = getHist(hN2,['DY_Sherpa'],fHist,lumiScales)
          hDY3 = getHist(hN3,['DY_Sherpa'],fHist,lumiScales)
	  ...
	  tmp = getHistIntegral(hZH[y])
        ...
        if r.split('_')[1] != '2bjet':
          plots_process = [hDat[y].Clone().Rebin(nRebin),hST[y].Clone().Rebin(nRebin),hVV[y].Clone().Rebin(nRebin),hZH[y].Clone().Rebin(nRebin),hTT[y].Clone().Rebin(nRebin),hDY[y].Clone().Rebin(nRebin)]
        else:
          plots_process = [hDat[y].Clone().Rebin(nRebin),hST[y].Clone().Rebin(nRebin),hVV[y].Clone().Rebin(nRebin),hZH[y].Clone().Rebin(nRebin),hTT[y].Clone().Rebin(nRebin),hDY3[y].Clone().Rebin(nRebin),hDY2[y].Clone().Rebin(nRebin),hDY1[y].Clone().Rebin(nRebin)]
        plotNames_process = []
        #dataTitle = 'Data ('
	dataTitle = 'Data '
        if r == 'Z_jet' and chan == 'Zee': decay_channel = 'Z(#rightarrowee) + #geq 1 jet'#+ 'Z(ee) + jets)' 
        if r == 'Z_jet' and chan == 'Zmm': decay_channel = 'Z(#rightarrow#mu#mu) + #geq 1 jet' #+ 'Z(#mu#mu) + jets)'
        if r == 'Z_bjet' and chan == 'Zee': decay_channel = 'Z(#rightarrowee) + #geq 1 b jet' #+ 'Z(ee) + b jets)'
        if r == 'Z_bjet' and chan == 'Zmm': decay_channel = 'Z(#rightarrow#mu#mu) + #geq 1 b jet' #+ 'Z(#mu#mu) + b jets)'
        if r == 'Z_2bjet' and chan == 'Zee': decay_channel = 'Z(#rightarrowee) + #geq 2 b jets' #+ 'Z(ee) + 2 b jets)'
        if r == 'Z_2bjet' and chan == 'Zmm': decay_channel = 'Z(#rightarrow#mu#mu) + #geq 2 b jet' #+ 'Z(#mu#mu) + 2 b jets)'
        plotNames_process = [dataTitle, 'Single top', 'Diboson', 'ZH','t#bar{t}', 'Z+jets']
        if r.split('_')[1] == '2bjet':
          plotNames_process = [dataTitle, 'Single top', 'Diboson','ZH','t#bar{t}', 'Z+XX', 'Z+bX', 'Z+bb']

        utl_func.makeStackPlot(plots_process, plotNames_process, plN + '_' + r + '_'+chan+'_'+y+'_'+dy_type, plotFolder + '/20'+y, xA_title, xA_range, 'MC unc. (stat)', False, lumi=lumiS[y],channel = decay_channel)
      ...
      if r.split('_')[1] != '2bjet':
        plots_process = [hDatA.Clone().Rebin(nRebin),hSTA.Clone().Rebin(nRebin),hVVA.Clone().Rebin(nRebin),hZHA.Clone().Rebin(nRebin),hTTA.Clone().Rebin(nRebin),hDYA.Clone().Rebin(nRebin)]
      else:
        plots_process = [hDatA.Clone().Rebin(nRebin),hSTA.Clone().Rebin(nRebin),hVVA.Clone().Rebin(nRebin),hZHA.Clone().Rebin(nRebin),hTTA.Clone().Rebin(nRebin),hDY3A.Clone().Rebin(nRebin),hDY2A.Clone().Rebin(nRebin),hDY1A.Clone().Rebin(nRebin)]
      
      utl_func.makeStackPlot(plots_process, plotNames_process, plN + '_' + r + '_'+chan+'_All_'+dy_type, plotFolder + '/All', xA_title, xA_range, 'MC unc. (stat)', False, lumi='137.1',channel = decay_channel)
